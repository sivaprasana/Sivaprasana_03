<h1 align="center">Hi 👋, I'm Sivaprasana </h1>
<h3 align="center">MCA pursuing Student | Aspiring Software Developer & Data Scientist | Python Enthusiast</h3>

<p align="center">
  Turning data into insights and ideas into working code — currently seeking my first full-time role in Software Development / Data Science.
</p>

<p align="center">
  <img src="https://img.shields.io/badge/Status-Open%20to%20Work-brightgreen?style=flat-square" alt="Open to Work"/>
  <img src="https://img.shields.io/badge/Fresher-MCA%20Student-blue?style=flat-square" alt="Fresher"/>
  <img src="https://img.shields.io/badge/Focus-Python%20%7C%20Data%20Science-orange?style=flat-square" alt="Focus"/>
</p>

---

### 👨‍💻 About Me

I'm an MCA pursuing student at Anna University (Distance Education) and a BCA graduate from Alpha Arts and Science College, Chennai, with a strong foundation in Python programming, data analysis, and machine learning. During my internship as a Data Science Intern, I worked hands-on with real datasets, building models and extracting insights using tools like Pandas, NumPy, and Scikit-learn. I enjoy solving problems, learning new technologies, and building projects that connect theory with practical, real-world impact. I'm now looking to bring that curiosity and work ethic to a full-time role in Software Development, Python programming, or Data Science.

---

### 🛠️ Skills

**Programming Languages**
<br/>
![Python](https://img.shields.io/badge/-Python-3776AB?style=flat-square&logo=python&logoColor=white)
![Java](https://img.shields.io/badge/-Java-007396?style=flat-square&logo=java&logoColor=white)
![C++](https://img.shields.io/badge/-C++-00599C?style=flat-square&logo=cplusplus&logoColor=white)
![PL/SQL](https://img.shields.io/badge/-PL%2FSQL-F80000?style=flat-square&logo=oracle&logoColor=white)

**Data Science & Machine Learning**
<br/>
![Pandas](https://img.shields.io/badge/-Pandas-150458?style=flat-square&logo=pandas&logoColor=white)
![NumPy](https://img.shields.io/badge/-NumPy-013243?style=flat-square&logo=numpy&logoColor=white)
![Scikit-learn](https://img.shields.io/badge/-Scikit--learn-F7931E?style=flat-square&logo=scikitlearn&logoColor=white)

**Databases & Tools**
<br/>
![MongoDB](https://img.shields.io/badge/-MongoDB-47A248?style=flat-square&logo=mongodb&logoColor=white)
![Node.js](https://img.shields.io/badge/-Node.js-339933?style=flat-square&logo=nodedotjs&logoColor=white)
![Oracle](https://img.shields.io/badge/-Oracle-F80000?style=flat-square&logo=oracle&logoColor=white)
![Excel](https://img.shields.io/badge/-MS%20Excel-217346?style=flat-square&logo=microsoftexcel&logoColor=white)

**Core Strengths**
- Data Analysis & Preprocessing
- Machine Learning Fundamentals
- Problem Solving & Logical Thinking
- Software Development Basics
- Fast, Continuous Learner

---

### 💼 Internship Experience

**Data Science Intern — Gardtwin Services Pvt. Ltd**
*October 2024 – February 2025*

- Worked on real-world data science and machine learning tasks under practical business scenarios
- Cleaned, processed, and analyzed complex datasets using **Pandas** and **NumPy**, improving overall data quality
- Built and evaluated machine learning models using **Scikit-learn** to support business decision-making
- Gained hands-on experience translating raw data into actionable insights

---

### 🚀 Projects

#### 🔍 Crime Prevention Through CCTV Network
A real-time monitoring and detection system focused on identifying suspicious activity through CCTV network analysis.

- **Key Features:** Real-time video processing, activity/anomaly detection, security-focused alert logic
- **Tech Used:** `Python`, `OpenCV`
- **What I Learned:** Practical exposure to computer vision concepts, real-time processing pipelines, and applying detection logic to security use cases

#### 📈 Stock Market Prediction
A machine learning based project to analyze historical stock data and forecast market trends.

- **Key Features:** Trend analysis, predictive modeling, data-driven forecasting
- **Tech Used:** `Python`, `Pandas`, `Scikit-learn`
- **What I Learned:** End-to-end ML workflow — from data preprocessing to model building and evaluating prediction performance on real financial data

---

### 🎓 Education

| Degree | Institution | Status |
|---|---|---|
| **MCA** – Computer Applications | Anna University (Distance Education), Guindy | Pursuing (Nov 2027) |
| **BCA** – Computer Applications | Alpha Arts and Science College, Porur, Chennai | Completed (Jul 2025) |

---

### 📚 Currently Learning

- Advanced Machine Learning techniques & data mining
- Building stronger project portfolios with Python & ML
- Exploring MongoDB and Node.js for full-stack fundamentals

---

### 🎯 Career Objective / Current Focus

I'm actively looking for entry-level opportunities in:

- 💻 Software Development
- 🐍 Python Developer roles
- 📊 Data Analyst / Data Science (Entry-Level)
- 🖥️ IT Fresher Roles

---

### 🌱 Open to Opportunities

I'm currently **open to full-time opportunities** as a fresher in Software Development, Python Development, or Data Science/Analytics roles. If you're hiring or know of a good fit, I'd love to connect!

---

### 📊 GitHub Stats

<p align="center">
  <img src="https://github-readme-stats.vercel.app/api?username=YOUR_GITHUB_USERNAME&show_icons=true&theme=default" alt="GitHub Stats" height="165"/>
  <img src="https://github-readme-streak-stats.herokuapp.com/?user=YOUR_GITHUB_USERNAME&theme=default" alt="GitHub Streak" height="165"/>
</p>

<p align="center">
  <img src="https://github-readme-stats.vercel.app/api/top-langs/?username=YOUR_GITHUB_USERNAME&layout=compact&theme=default" alt="Top Languages"/>
</p>

> ⚠️ Replace `YOUR_GITHUB_USERNAME` above with your actual GitHub username to activate these stats.

---

### 📬 Connect With Me

- 💼 LinkedIn: [Add your LinkedIn link]
- 📧 Email: [Add your email]
- 🌐 Portfolio: [Optional — add your portfolio site if you have one]
- 🐙 GitHub: [Add your GitHub username]

---

<p align="center"><i>Thanks for stopping by! Always happy to connect, collaborate, or chat about Python, data, and building things that work. 🚀</i></p>
